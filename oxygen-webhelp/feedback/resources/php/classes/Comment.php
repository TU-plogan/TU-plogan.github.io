...
// 				$db->Run($query0);
            ...
            while ($db->MoveNext()) {
                ...
                if (($db->Field('userId') == $idUser && $state == 'new') || $isForAdmin || $state == 'approved') {
                    ...
                    if ($isForAdmin) {
                        ...
                        if ($state != "approved") {
                            $toReturn .= "<button type='button' class='btn btn-default btn-sm bt_delete' title='" . Utils::translate("delete") . "' data-comment-id='".$parent."'>" . Utils::translate("delete") . "</button>";
                            $toReturn .= "<button type='button' class='btn btn-default btn-sm bt_edit' title='" . Utils::translate("edit") . "' data-comment-id='".$parent."'>" . Utils::translate("edit") . "</button>";
                            $toReturn .= "<button type='button' class='btn btn-default btn-sm bt_approve' title='" . Utils::translate("approve") . "' data-comment-id='".$parent."'>" . Utils::translate("approve") . "</button>";
                        } else {
                            if ($state == "approved") {
                                $toReturn .= "<button type='button' class='btn btn-default btn-sm bt_delete' title='" . Utils::translate("delete") . "' data-comment-id='".$parent."'>" . Utils::translate("delete") . "</button>";
                                $toReturn .= "<button type='button' class='btn btn-default btn-sm bt_edit' title='" . Utils::translate("edit") . "' data-comment-id='".$parent."' >" . Utils::translate("edit") . "</button>";
                    ...
                    if ((($idUser == $db->Field('userId') && $state == 'new') && !$isForAdmin) || ($state == 'approved' && !$isForAdmin))
                    {
                        if (!$this->isAnonymous($idUser) && $idUser == $db->Field('userId')) {
                            $toReturn .= "<div class=\"actions\">";
                            $toReturn .= "<button type='button' class='bt_delete btn btn-default btn-sm' title='" . Utils::translate("delete") . "' data-comment-id='".$parent."'>" . Utils::translate("delete") . "</button>";
                            $toReturn .= "<button type='button' class='bt_edit btn btn-default btn-sm' title='" . Utils::translate("edit") . "' data-comment-id='".$parent."' >" . Utils::translate("edit") . "</button>";
                            $toReturn .= "</div>";
                        }
                    }
                    ...
                    if ($state == "approved") {
                        ...
                        $toReturn .= "<span class='bt_reply' data-comment-id='".$parent."'>" . Utils::translate("reply") . "</span>";
                        $toReturn .= "</div>";
                    }
                    if ($mustModerate) {
     ...
     *
     * @param $product String product to query versions for
     * @param $version String current product version
    ...
    function queryVersions($product, $version)
    {
        ...
        // get all versions and assume that all are visible by default
        $query = "SELECT DISTINCT(version) FROM webhelp;";
        if ($db->Open($query) > 0) {
            while ($db->MoveNext()) {
                $toReturn[$db->Field('version')] = "true";
            }
        }
