    ...
    if (Utils::isLoggedModerator($pName, $pVersion)) {
        ...
        if (count($vList) > 0) {
            ...
            foreach ($vList as $origProduct => $versions) {
                $product = $origProduct;
                $toPrint .= "<div class='p_selectable' id='p_$idx' data-id='" . $idx . "' data-product='" . addslashes($product) . "'>" . $product . "</div>";
                $idx++;
            }
            $toPrint .= "</div>";
            $toPrint .= "</div>";
            ...
            foreach ($vList as $origProduct => $versions) {
                ...
                foreach ($versions as $version) {
                    $toPrint .= "<div id='ver_" . $idx . "_" . $vidx . "' class='selectable' data-version='" . addslashes($version) . "');\">" . $version . "</div>";
                    $vidx++;
                }
                $toPrint .= "</div>";
                $idx++;
