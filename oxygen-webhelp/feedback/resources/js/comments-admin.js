...
define(["jquery", "whf_config", "init", "localization", "debug", "base64", "bootstrap", "jquery.quicksearch"],
...
//var pageWSearch = pageSearch.replace(location.search, "");
        ...
        function showAdmin() {
            ...
            $.ajax({
                ...
                success: function (data_response) {
                    ...
                    $('input#id_search').quicksearch('table#usersList tbody tr');
                    $('table#usersList tbody tr').on("click", function(){
                ...
                success: function (data_response) {
                    ...
                    if (response.loggedIn == 'true') {
                        ...
                        if ($("html").attr("dir") != "rtl") {
                            $("#accountInfo").html(i18n.getLocalization("label.welcome") + " " + i18n.getLocalization("label.guest"));
                        } else {
                            $("#accountInfo").html(i18n.getLocalization("label.guest") + " " + i18n.getLocalization("label.welcome"));
                        }
        ...
        function loggedUser(response) {
            if ($("html").attr("dir") != "rtl") {
                $("#accountInfo").html(i18n.getLocalization("label.welcome") + " " + response.name + " [" + response.userName + "]");
            } else {
                $("#accountInfo").html("[" + response.userName + "] " + response.name + " " + i18n.getLocalization("label.welcome"));
            }
            ...
            var $goBackButton = $('<button>').attr('type', 'button')
                                        .addClass('bt_toolbar').addClass('btn').addClass('btn-default').addClass('btn-primary')
                                        .html(i18n.getLocalization('label.back'))
                                        .on('click', function(){
                                            goLastPage();
                                        });
            $("#accountInfo").append($goBackButton);
...
// logoff form execute
                    ...
                    success: function (data_response) {
                        ...
                        if (typeof pageWSearch !== 'undefined' && window.location.href != pageWSearch) {
                            if (response.authenticated == 'false') {
                                showLoggInDialog();
                                if (response.error) {
                                    var msg = i18n.getLocalization('checkUser.loginError');
                ...
                success: function (data_response) {
                    if (data_response == 'USER_SESSION_ERROR') {
                        ...
                        if (response.versions == "") {
                            $('#setVersionInfo').html(i18n.getLocalization("info.noComments"));
                            $('#versions').html("");
                        } else {
                            ...
                            $('#versions .clickable').on("click", function(){
            ...
            $.ajax({
                ...
                success: function (data_response) {
                    if (data_response == 'USER_SESSION_ERROR') {
                        ...
                        if (data_response == "") {
                            $('#ex_prod_val').html("<div class='listTitle'>" + i18n.getLocalization("info.noComments") + "</div>");
                        } else {
                            $('#ex_prod_val').html(data_response);
                            $('#ex_prod_val').append("<div id='empty_versions'>&nbsp;</div>");
                            $('#ex_prod_val .products .p_selectable').on("click", function(){
                                var $idx = $(this).attr('data-id');
                                var $product = $(this).attr('data-product');
                                showVersions($idx,$product);
                            });

                            $('#ex_prod_val .product_Versions .selectable').on("click", function(){
                                var $version = $(this).attr('data-version');
                                setExpVersion(this,$version);
                            });
                        }
                        $('#preload').hide();
        ...
        function viewAllPosts() {
            ...
            $.ajax({
                ...
                success: function (data_response) {
                    $('#preload').hide();
                    $('#ll_viewAll_tit_info').html("&nbsp;&nbsp;&nbsp;" + i18n.getLocalization('label.product') + ": " + productN + "&nbsp;&nbsp;&nbsp;" + i18n.getLocalization('label.version') + ": " + productV + " ");
                    $('#inlineViewDiv #v_preload').hide();
                    $('#ex_inline').html(data_response);
                    $('#ex_inline').find('input.cb-element').on("click", function(){
                        var $id = $(this).val();
                        addToDelete($id);
                    });
                }
        ...
        function addInfoDialog() {
            $('#inlineViewDiv').append("<div id='info_container' style='display: block; position: absolute; top: 150px; width: 90%; margin:0 5%; background: #fffff0;'><div id='info_dialog'>" + i18n.getLocalization('label.confirmOk') + "</div></div>");
        ...
        function cleanUsersDb() {
            ...
            $('#ll_viewAll_tit').html(i18n.getLocalization('label.unconfirmedUsers'));
            $('#ll_viewAll_tit_info').html("&nbsp;&nbsp;&nbsp;" + i18n.getLocalization('label.product') + ": "
                + productName + "&nbsp;&nbsp;&nbsp;" + i18n.getLocalization('label.version') + ": " + productVersion + " ");
            $('#inlineViewDiv #v_preload').show();
            $.ajax({
                type: "POST",
                url: setVersionPath,
        ...
        function cleanCommentsDb() {
            ...
            $('#ll_viewAll_tit').html(i18n.getLocalization('label.invalidPosts'));
            $('#ll_viewAll_tit_info').html("&nbsp;&nbsp;&nbsp;" + i18n.getLocalization('label.product') + ": "
                + productName + "&nbsp;&nbsp;&nbsp;" + i18n.getLocalization('label.version') + ": " + productVersion + " ");
            $('#inlineViewDiv #v_preload').show();
            $.ajax({
        ...
        $("#bt_viewPosts").click(function () {
            ...
            $('#ll_exp_tit').html(i18n.getLocalization('label.forView'));
            showExportComments();
        });
        ...
        $("#bt_export").click(function () {
            ...
            $('#ll_exp_tit').html(i18n.getLocalization('label.forExport'));
            showExportComments();
        });
        ...
        return {
            ...
            cleanDeleteUsr : cleanDeleteUsr,
            deleteCmts : deleteCmts,
            cleanDeleteCmts : cleanDeleteCmts,
